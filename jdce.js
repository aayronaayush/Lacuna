...
function get_analyzer_data(filter) {
	...
	filter.forEach(function (name) {
		if (available_analyzers.indexOf(name) != -1) {
			...
			console.log(
				"Note: analyzer '" + name + "' not found in analyzers/"
			);
		}
	});
...
function remove_functions_from_file(file_name, functions, url) {
	...
	functions.forEach(function (func) {
		...
		source_code = source_code.splice(
			func.start - offset,
			func.end - func.start,
			insert
		);
...
function remove_nested_functions(file_name, functions) {
	...
	for (var i = 0; i < functions.length; i++) {
		...
		for (var j = 0; j < functions.length; j++) {
			if (
				functions[i].start > functions[j].start &&
				functions[i].end < functions[j].end
			) {
				nested = true;
...
module.exports = {
	run: async function (settings, callback) {
		...
		let nodes = GraphTools.build_function_graph(
			scripts,
			CONSTRUCTED_EDGE.value
		);
		...
		let analyzer_settings = {
			...
			error_handler: function (name, message) {
				if (settings.missteps) {
					console.error(
						"Analyzer '" + name + "' encountered an error:",
						message
					);
				}
			},
		...
		console.log(
			...
			analyzer_settings.base_node.get_edges().length,
			new Date()
		);
		if (analyzers.functions.length > 0) {
			...
			async_loop(
				analyzers.functions,
				analyzer_settings,
				function (analyzer_run_info) {
					stats.analyzer_info = analyzer_run_info
		...
		function process_marked_graph() {
			...
			if (!settings.noremove) {
				nodes = GraphTools.remove_constructed_edges(
					nodes,
					CONSTRUCTED_EDGE.value
				);

				let disconnected_nodes = GraphTools.get_disconnected_nodes(
					nodes
				);
			...
			if (settings.graph) {
				...
				if (settings.show_disconnected) {
					stats.graph = GraphTools.output_function_graph(
						nodes,
						analyzers.fingerprints
					);
				} else {
					// Only show reachable nodes.
					stats.graph = GraphTools.output_function_graph(
						GraphTools.get_connected_nodes(nodes),
						analyzers.fingerprints
					);
				}
			}
		...
		function return_results() {
			...
			stats.run_time = parseInt(
				((end_time[0] * 1e9 + end_time[1]) * 1e-6).toFixed(0),
				10
			);
