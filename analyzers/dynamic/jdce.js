...
module.exports = {
	...
	run: async function (settings, callback) {
		...
		function print_analytics(results) {
			for (const file in script_editors) {
				if (script_editors[file].functions != null) {
					const [
						functionsNotCalledInFile,
						totalWordCount,
					] = get_functions_not_called_in_file(results, file);
					DBModelMySql.setFileStats(file, {
						totalFunctions: script_editors[file].functions.length,
						updatedFunctions:
							script_editors[file].functions.length -
							functionsNotCalledInFile.length,
					});
					console.log(
						`In ${file}, there were ${script_editors[file].functions.length} functions in total. ${functionsNotCalledInFile.length} were removed, saving ${totalWordCount} bytes.`
					);
				}
			}
		}
		...
		function get_functions_not_called_in_file(results, file) {
			...
			return [
				script_editors[file].functions.filter(
					functionsNotCalledFilter(file)
				),
				totalWordCount,
			];
		}
